...
- 데이터를 바꾸는 동작(생성·수정·삭제)은 **API 라우트**로 만든다. 화면을 거치지 않고
  API를 직접 호출해도 막히는지 테스트로 확인할 수 있어야 하기 때문이다.
  로그인/로그아웃처럼 세션 쿠키를 다루는 것만 서버 액션을 쓴다.
- 드롭다운은 기본 `<select>`(`components/ui/native-select.tsx`)를 쓴다.
  휴대폰에서 OS 기본 UI가 떠서 쓰기 편하고 코드가 단순하다.
